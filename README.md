# ARTC Auto_script - 晶胞结构力学性能自动化测试系统

基于 Abaqus 有限元分析的晶胞结构力学性能自动化测试系统，支持 20 种不同晶胞结构的静态/动态压缩、剪切和方向性测试，实现从脚本生成、批处理执行到数据后处理的完整工作流。
...
本系统为材料力学研究提供自动化仿真工具链，主要功能包括：
- **脚本生成**：基于 PyQt5 的可视化界面，批量生成 Abaqus 有限元分析脚本
- **批处理执行**：支持 Windows/Linux 跨平台批处理，集成 SLURM/PBS 集群调度
- **数据后处理**：自动提取力-位移曲线，生成标准化 JSON 数据集
- **可视化分析**：多维度曲线对比与性能指标统计

**支持的晶胞结构**（20 种）：
- 基础结构：Cubic, BCC, FCC, Diamond
- 变体结构：BCCZ, FCCZ, AFCC, FBCCZ, FBCCXYZ
- 复杂结构：Kelvin, Octet_truss, Iso_truss, Auxetic
- 其他结构：Tetrahedron_base, G7, Cuboctahedron_Z, Rhombic, Octahedron, Truncated_cube, Truncated_Octoctahedron
...
- ✅ **动态接触面检测**：自适应识别 20 种晶胞结构的上下接触面，消除硬编码
- ✅ **三阶段执行流程**：预处理（生成 .inp）→ 求解器计算 → 后处理（提取数据）
- ✅ **跨平台批处理**：Windows .bat / Linux .sh 双平台支持
- ✅ **集群调度集成**：自动生成 SLURM/PBS 作业提交脚本
- ✅ **智能断点续算**：检查现有数据，自动跳过已完成任务
- ✅ **数据标准化**：力-位移曲线插值到统一采样点（50/500 点）

### 测试模式
- **Static 模式**：隐式静态分析，适用于准静态压缩测试
- **Speed 模式**：显式动态分析，支持速度参数（10/50/100/500 mm/s）
- **Direction 模式**：方向性测试，支持 X/Z 方向扫描
